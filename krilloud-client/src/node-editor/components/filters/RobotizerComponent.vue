...
<div class="row q-pt-xs q-pt-sm" @pointerdown.stop="" @click="$helper.callNodeSelected(node)">
    ...
    <div class="slider-title-mixer row col-10">
      <div class="q-pt-xs slider-title col-8">Frequency</div>
          <q-badge class="slider-value col-4" color="white" text-color="black">
              {{ isVisible(1) ? frequency:''}}
            </q-badge>
          <q-slider
              v-model="frequency"
              :min="parameters.frequency.minValue"
              :max="parameters.frequency.maxValue"
              :step="0.100"
...
<div class="row q-pt-xs q-pt-sm" @pointerdown.stop="" @click="$helper.callNodeSelected(node)">
    ...
    <div class="slider-title-mixer row col-10">
      <div class="q-pt-xs slider-title col-8">Waveform</div>
          <q-badge class="slider-value col-4" color="white" text-color="black">
              {{ isVisible(2) ? waveform:''}}
            </q-badge>
          <q-slider
              v-model="waveform"
              :min="parameters.waveform.minValue"
              :max="parameters.waveform.maxValue"
              :step="0.100"
...
export default {
  ...
  computed: {
    ...
    frequency: {
      get () {
        return parseFloat(this.datafilter.frequency.value.toFixed(3))
      },
      set (v) {
        this.$store.dispatch('main/setFilterParameter', { id: 1, value: v, tagname: this.currentTagName, idNode: this.idNode, name: 'frequency', prevValue: this.datafilter.frequency.value })
        this.datafilter.frequency.value = v
      }
    },
    waveform: {
      get () {
        return parseFloat(this.datafilter.waveform.value.toFixed(3))
      },
      set (v) {
        this.$store.dispatch('main/setFilterParameter', { id: 2, value: v, tagname: this.currentTagName, idNode: this.idNode, name: 'waveform', prevValue: this.datafilter.waveform.value })
        this.datafilter.waveform.value = v
      }
    }
  }
}
