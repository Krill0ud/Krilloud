...
<div class="row q-pt-xs q-pt-sm" @pointerdown.stop="" @click="$helper.callNodeSelected(node)">
    ...
    <div class="slider-title-mixer row col-10">
      <div class="q-pt-xs slider-title col-8">Amount</div>
          <q-badge class="slider-value col-4" color="white" text-color="black">
              {{ isVisible(1) ? amount:''}}
            </q-badge>
          <q-slider
              v-model="amount"
              :min="parameters.amount.minValue"
              :max="parameters.amount.maxValue"
              :step="0.001"
...
export default {
  ...
  computed: {
    ...
    amount: {
      get () {
        return parseFloat(this.datafilter.amount.value.toFixed(3))
      },
      set (v) {
        this.$store.dispatch('main/setFilterParameter', { id: 1, value: v, tagname: this.currentTagName, idNode: this.idNode, name: 'amount', prevValue: this.datafilter.amount.value })
        this.datafilter.amount.value = v
      }
    }
  }
}
