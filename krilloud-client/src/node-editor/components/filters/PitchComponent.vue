...
<!-- Inputs-->
      <div class="q col-1 input" :class="$helper.socketConnectedClass(inputAudio.connections, inputAudio.socket.name)">
...
<div class="row q-pt-xs q-pt-sm" @pointerdown.stop="" @click="$helper.callNodeSelected(node)">
    ...
    <div class="slider-title-mixer row col-10">
      <div class="q-pt-xs slider-title col-8">Tune</div>
          <q-badge class="slider-value col-4" color="white" text-color="black">
              {{ isVisible() ? tune:''}}
            </q-badge>
          <q-slider
              v-model="tune"
              :min="parameters.tune.minValue"
              :max="parameters.tune.maxValue"
              :step="0.001"
...
export default {
  ...
  computed: {
    ...
    tune: {
      get () {
        return parseFloat(this.datafilter.tune.value.toFixed(3))
      },
      set (v) {
        this.$store.dispatch('main/setFilterParameter', { id: 0, value: v, tagname: this.currentTagName, idNode: this.idNode, name: 'tune', prevValue: this.datafilter.tune.value })
        this.datafilter.tune.value = v
      }
    }
  }
}
